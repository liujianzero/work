...
class PayController extends UserCenterController
{
        ...
        if ($order) {
            try {
                ...
                switch ($order->buy_type) {
                    case 'goods':
                        $type = ModelsOrderModel::where('id', $order->member_id)->value('transaction_mode');
                        switch ($type) {
                            case '1':// 出售商品
                                $url = route('myOrder.myGoodsOut');
                                break;
                            case '2':// 查看付费
                                $url = route('myOrder.myViewOut');
                                break;
                            case '3':// 出售素材
                                $url = route('myOrder.myMaterialOut');
                                break;
                            case '4':// 定制服务
                                $url = route('myOrder.myTaskOut');
                                break;
                            default:
                                $url = route('indexPage');
                                break;
                        ...
                        break;
                    case 'study':
                        $url = route('video', ['id', $order->member_id]);
                        break;
                    case 'task_merge':
                    case 'task_bounty':
                    case 'task_server':
                        $url = route('myTasksList');
                        break;
                    case 'store_renew':
                        $url = route('shop.list');
                        break;
    ...
    public function weChatPayStatus(Request $request)
    {
        ...
        if ($response->isSuccessful() && $response->isTradeStatusOk()) {
            ...
            switch ($type) {
                case 'buy member':
                    OrderModel::thirdPayBuyMember(3,$data);
                    break;
                case 'buy study':
                    OrderModel::thirdPayBuyStudy(3,$data);
                    break;
                case 'buy capacity':
                    OrderModel::thirdPayBuyCapacity(3,$data);
                    break;
                case 'recharge':
                    OrderModel::thirdPayRecharge(3, $data);
                    break;
                case 'buy goods':
                    OrderModel::thirdPayBuyGoods(3, $data);
                    break;
                case 'task_merge':
                case 'task_bounty':
                case 'task_server':
                    OrderModel::thirdPayTaskBounty(3, $data);
                    break;
                case 'store_renew':
                    OrderModel::thirdPayStoreRenew(3, $data);
                    break;
                default:
                    # code...
                    break;
            }
            Session::flash('message', '支付成功');
    ...
    public function getFinanceList(Request $request)
    {
        ...
        if ($request->get('type')){
            switch ($request->get('type')){
                case 'cash':
                    $list = FinancialModel::where('uid', $user->id)->where('action', '>',10)->orderBy('created_at', 'desc')->paginate(10);
                    break;
                case 'cashout':
        ...
        $data = [
            'balance' => floatval($userDetail->balance),
            'list' => $list,
            'type' => $request->get('type') ? $request->get('type') : ''
        ];

        /* @author orh @time 2017-08-01 @edit start */
        $this->initTheme ( 'userinfo' ); // 主题初始化
        $this->theme->setTitle ( '资金明细' );
        /* @author orh @time 2017-08-01 @edit end */
    ...
    public function assetdetail(Request $request)
    {
        ...
        $cashIn = FinancialModel::where('uid', $user->id)->where('action', '>' ,10)->sum('cash');
        $cashOut = FinancialModel::where('uid', $user->id)->where('action', '<=',10)->sum('cash');
