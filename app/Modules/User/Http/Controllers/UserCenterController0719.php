...
class UserCenterController extends BasicUserCenterController {
	...
	public function index() {
		...
		//默认文件夹的作品总数
		$query = ModelsContentModel::where('uid', '=', $this->user ['id'] )->where('folder_id',0);
		$defaultFolderCount = $query->count();
		
		// 获取用户的文件夹
		$folderList = ModelsFolderModel::select ( 'id', 'name', 'cover_img', 'auth_type', 'update_time', 'create_time' )->where ( 'uid', '=', $this->user ['id'] )->orderBy ( 'create_time', 'desc' )->limit ( 10 )->get ();
		
		foreach ( $folderList as &$v ) {
			$v ["count"] = ModelsContentModel::where ( 'uid', '=', $this->user ['id'] )->where ( 'folder_id', $v ["id"] )->count ();
		}
	...
	public function folder($id = 0) {
		...
		if ($id != 0) {
			...
			$models = ModelsContentModel::select ( 'id', 'title', 'content', 'cover_img','upload_cover_image', 'create_time','is_private' )->where ( 'folder_id', '=', $id )->orderBy ( 'create_time', 'desc' )->get ();
			
			$folder = ModelsFolderModel::select ( 'id', 'name', 'cover_img', 'update_time', 'create_time' )->where ( 'id', '=', $id )->orderBy ( 'create_time', 'desc' )->first ();
		} else {
			// 获取默认文件夹下的所有作品
			$models = ModelsContentModel::select ( 'id', 'title', 'content', 'cover_img','upload_cover_image', 'create_time','is_private' )->where ( 'uid', '=', $this->user ['id'] )->where ( 'folder_id', '=', $id )->orderBy ( 'create_time', 'desc' )->limit ( 10 )->get ();
			$folder = null;
		}
		
		// 获取用户的所有文件夹
		$folderList = ModelsFolderModel::select ( 'id', 'name', 'cover_img', 'update_time', 'create_time' )->where ( 'id', '!=', $id )->where ( 'uid', '=', $this->user ['id'] )->orderBy ( 'create_time', 'desc' )->limit ( 10 )->get ();
	...
	public function zone($id = 0,$folder=0) {
		...
		//获取用户等级和经验值
		$user_data = UserModel::select ( 'users.id','users.experience', 'user_detail.avatar', 'user_detail.balance', 'user_detail.nickname', 'user_detail.introduce' )->where ( 'users.id', $id )->join ( 'user_detail', 'users.id', '=', 'user_detail.uid' )->first ();
...
//		}
		...
		$models = $models->where ( 'is_private', '=', 0 )->orderBy ( 'create_time', 'desc' )->paginate (18);
...
//				'empCmt'=>$empComments
	...
	public function deleteFolder(Request $request) {
		...
		$folder = ModelsFolderModel::select ( 'uid' )->where ( 'id', '=', $id )->first ();
