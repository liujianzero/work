...
//        exit;
        ...
        if($type == 'member'){  //会员类 1,会员只高不低
            ...
            echo '正在开发中';exit;
        } elseif ($type == 'task') {// 众包服务
            $id = intval($id);
            $userId = intval($userId);
            $typeId = intval($typeId);
            $model = TaskModel::where('id', $id)
                ->where('uid', $userId)
            ...
            if ($model) {
                ...
                $message = '未找到该订单信息或已完成支付';
            }
        } elseif ($type == 'store_renew') {// 店铺续费
            $model = UserModel::from('users as u')
                ->select([
                    'u.*',
                    'sc.store_name',
                ])
            ...
            if ($model) {
                ...
                $model->price = $years[$typeId];
                $title = "店铺「{$model->store_name}」续费 {$typeId} 年";
            } else {
                $message = '请选择要续费的店铺';
            }
        }else{
    ...
    static function createOrder($type,$typeId,$uid,$money,$title,$code,$goodsTypeId){
        $status = DB::transaction(function() use($type,$typeId,$uid,$money,$title,$code,$goodsTypeId){
            if($type == 'member'){  //会员类
                ...
                $action = 1;
                $result = OrderModel::balancePayTaskBounty(['code' => $code, 'money' => $money]);
                if (! $result) {
                    return false;// 操作失败
                }
                UserDetailModel::where('uid', $uid)->where('balance_status', '<>', 1)->decrement('balance', $money);
                ...
                $store = StoreConfig::where('store_id', $order->member_id)->first();
                if (! $store) {
                    return false;
                }
                if (! $store->expire_at || $store->expire_at < date('Y-m-d H:i:s')) {
                    $update['expire_at'] = date('Y-m-d H:i:s', strtotime("+{$order->task_id} year"));
                } else {
                    $update['expire_at'] = date('Y-m-d H:i:s', strtotime($store->expire_at . " +{$order->task_id} year"));
                }
