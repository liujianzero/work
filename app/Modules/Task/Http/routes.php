...
//	Route::get('/', 'IndexController@index');//本地测试
    ...
    Route::post('/ajax/category/{id}','IndexController@taskCategory')->name('task.ajax.category');// 发布任务-获取分类@ajax
    Route::post('/ajax/region/{id}','IndexController@getRegion')->name('task.ajax.region');// 发布任务-获取地区数据@ajax
    Route::post('/ajax/content/upload','IndexController@uploadContentImages')->name('task.ajax.content.upload');// 发布任务-富文本上传图片@ajax
    Route::post('/ajax/file/upload','IndexController@taskUpload')->name('task.ajax.file.upload');// 发布任务-附件上传@ajax
    Route::post('/ajax/file/del/{id}','IndexController@taskDelFile')->name('task.ajax.file.del');// 发布任务-删除文件@ajax
    Route::post('/ajax/bounty/limit','IndexController@getTaskBountyLimit')->name('task.ajax.bounty.limit');// 发布任务-任务最大/最小金额@ajax
    Route::post('/ajax/validate/from','IndexController@ajaxValidateFrom')->name('task.ajax.validate.from');// 任务表单验证@ajax
    Route::get('/update/{id}','IndexController@update')->name('taskUpdatePage');//创建编辑页面
    Route::post('/updateTask','IndexController@updateTask')->name('updateTask');//编辑任务提交
    Route::post('/ajax/models/get','DetailController@modelsGet')->name('task.ajax.models.get');//获取文件夹作品@ajax
});
