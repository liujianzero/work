...
class TaskRightsModel extends Model
{
    ...
    public static function rightCreate($data)
    {
        $status = DB::transaction(function() use($data){
            self::create($data);
            WorkModel::where(['task_id' => $data['task_id'], 'status' => 2])
                ->whereIn('uid', [$data['from_uid'], $data['to_uid']])
                ->update(['status' => 4]);
            $worker_num = TaskModel::where('id', $data['task_id'])->value('worker_num');
            if ($worker_num == 1) {
                TaskModel::where('id', $data['task_id'])
                    ->update(['status' => 10, 'end_at' => date('Y-m-d H:i:s')]);
            } else {
                ...
                if ($work_checked_works == 0 && $work_checked == 0) {
                    TaskModel::where('id', $data['task_id'])->update(['status' => 10]);
                } elseif ($work_checked == 0){
                    TaskModel::where('id', $data['task_id'])
                        ->update(['status' => 7,'comment_at' => date('Y-m-d H:i:s')]);
                }
            }
        });
