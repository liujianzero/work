...
class GoodsModel extends Model
{
    ...
    protected $fillable = [
        ...
        'uid', 'shop_id', 'cate_id','mid', 'title', 'desc', 'unit', 'type', 'cash', 'cover', 'status', 'is_recommend','recommend_end',
    ...
    {
        $service = self::select('goods.*','us.name','models_content.cover_img')->where('goods.uid',$uid)->where('goods.type',2)->where('goods.is_delete',0);
        //状态筛选
        ...
        $service = $service->leftjoin('users as us','us.id','=','goods.uid')->leftJoin('models_content','models_content.id','=','goods.mid')
    ...
    static public function getGoodsListByUid($uid,$merge=array())
    {
        ...
        $goodsList = $goodsList->where('goods.uid',$uid)->where('goods.type',1)->where('is_delete',0)
            ->leftJoin('cate','cate.id','=','goods.cate_id')->leftJoin('models_content','models_content.id','=','goods.mid')
            ->select('goods.*','cate.name','models_content.cover_img')
     ...
     * @return mixed
     */
    static public function getPaidViewGoodsListByUid($uid,$merge=array())
    {
    	$goodsList = GoodsModel::whereRaw('1 = 1');
    	//状态筛选
    	if(isset($merge['status'])){
    	...
    	$goodsList = $goodsList->where('goods.uid',$uid)->where('goods.type',3)->where('is_delete',0)
    	->leftJoin('cate','cate.id','=','goods.cate_id')->leftJoin('models_content','models_content.id','=','goods.mid')
    	->select('goods.*','cate.name','models_content.cover_img')
    	->orderBy('goods.created_at','DESC')
    	->paginate(5);
