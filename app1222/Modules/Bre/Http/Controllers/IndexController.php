...
use App\Modules\Bre\Model\MoocRemarkModel;
...
//        header("Content-type:text/html;charset=utf-8");
    ...
    public function getVideo($id, $type){
        if($id){
            if(Auth::check()){
                ...
                if(intval($id) <= $num && intval($id) != 0 ){
                    ...
                    if(!empty($orderData)){
                        ...
                        $pcType = MoocPriceModel::where('id',$id)->value('type');
                        $url = MoocChapterModel::where('mooc_content.type_id',$id)
                            ->where('mooc_content.type_chapter', $type)
                            ->join('mooc_content','mooc_content.id','=','mooc_chapter.type_id')
                            ->pluck('url');
                        $chapterCount = MoocChapterModel::where('mooc_content.type_id',$id)
                            ->join('mooc_content','mooc_content.id','=','mooc_chapter.type_id')
                            ->count();

                        $fans_num = MoocRemarkModel::where('mooc_id',$id)->count();
                        $remarkList = MoocRemarkModel::select('user_detail.avatar','user_detail.nickname','mooc_remark.content','mooc_remark.id','mooc_remark.uid','mooc_remark.created_at')
                            ->where ( 'mooc_remark.mooc_id', $id )
                            ->where('mooc_remark.remark_id','mooc_remark.id')
                            ->join("user_detail","user_detail.uid","=","mooc_remark.uid")->orderBy('mooc_remark.created_at','desc')->get();
//                    dd($remarkList);exit;
                        if (!$remarkList->first()) {
//                    echo '空数据';exit;
                            $remark[] = '';
...
//                    echo '有数据';exit;
                            foreach($remarkList as $v){
                                $remark[] = array(
                                    'remark' => MoocRemarkModel::select('user_detail.avatar','user_detail.nickname','mooc_remark.content','mooc_remark.id','mooc_remark.uid','mooc_remark.created_at')
                                        ->where ( 'mooc_remark.mooc_id', $id )
                                        ->where('mooc_remark.remark_id','=',$v['id'])
                                        ->join("user_detail","user_detail.uid","=","mooc_remark.uid")->orderBy('mooc_remark.created_at','desc')
                                        ->orderBy('mooc_remark.created_at','desc')
                                        ->get()
                                );
                            }
                        }
    ...
    public function postRemark(Request $request) {
        ...
        $data = array (
            'uid'        => Auth::user()->id,
            'mooc_id'    => $id,
            'remark_id'  => $remark_id,//评论(子回复)ID
            'content'    => $content,
            'created_at' => date('Y-m-d H:i:s')
        );
//        dd($data);exit;
        $insert_id = MoocRemarkModel::insertGetId($data);
        $userData = UserDetailModel::where('uid',Auth::user()->id)->first();
        if(!$userData['avatar']){
