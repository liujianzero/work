...
class MyShopController extends UserCenterController
{
    ...
    public function getStoreList(Request $request)
    {
        if (Auth::check()) {
            $uid = Auth::user()->id;
            $list = UserModel::where('pid', $uid);
            if ($request->get('type')) {
                $list = $list->where('store_type_id', $request->get('type'));
            }
            $list = $list->latest()->paginate(9);
            if ($list->lastPage()) {
                $view = [
                   'list' => $list
                ];
     ...
     * 设计师通过切换店铺登录相应的店铺管理员
     */
    public function agentAdminLogin($id = 0)
    {
        if ($id <= 0) {
            return back()->with(['err' => '非法操作']);
        }
        $uid = Auth::user()->id;
        $agent = UserModel::where('id', $id)
            ->where('pid', $uid)
            ->first();
        if (! $agent) {
            return back()->with(['err' => '参数错误']);
