    ...
    public function alipayReturnHandle($type, $data)
    {
        switch ($type){
            ...
            case 'buy goods':
                ...
                break;
            case 'store_renew':
                $res = OrderModel::thirdPayStoreRenew(2, $data);
                if ($res) {
                    return redirect()->route('shop.list')->with(['message' => '支付成功！']);
                } else {
                    return redirect()->route('shop.list')->with(['message' => '支付失败！']);
                }
                break;
//            case 'buy other':
//                break;
...
//                break;
    ...
    public function alipayNotifyHandle($type, $data)
    {
        ...
        switch ($type){
            ...
            case 'task_bounty':
            case 'task_server':
                $res = OrderModel::thirdPayTaskBounty(2, $data);
                if ($res) {
                    exit('支付成功');
                }
                break;
...
//                break;
    ...
    public function wechatNotifyHandle($type, $data)
    {
        ...
        switch ($type){
            ...
            case 'task_bounty':
            case 'task_server':
                $status = OrderModel::thirdPayTaskBounty(3, $data);
                break;
            case 'store_renew':
                $status = OrderModel::thirdPayStoreRenew(3, $data);
                break;
//            case 'buy other':
//                break;
//            case 'cash':
