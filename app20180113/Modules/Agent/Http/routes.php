...
Route::group(['prefix' => 'agent'], function() {
    ...
    // 前台-自定义页面
    Route::get('/page/{store?}/{page?}','PagesController@show')->name('agent.pages.show');
    ...
    $group = \App\Modules\Agent\Model\StoreTemplate::getGroup();
    Route::group(['prefix' => 'shop'], function () use ($group) {
        foreach ($group as $v) {
            $config = strtolower($v->theme);
            $controller = $v->controller;
            Route::group(['prefix' => $config], function () use ($config, $controller) {
                Route::get('/{id}', $controller . '@' . $config)->name('agent.' . $config);// 博物馆-首页
                Route::get('/personal/{id}', $controller . '@personal')->name('agent.' . $config . '.personal');// 博物馆-个人中心
                Route::get('/summary/{id}', $controller . '@summary')->name('agent.' . $config . '.summary');// 博物馆-简介
                Route::get('/addr/{id}', $controller . '@address')->name('agent.' . $config . '.addr');// 博物馆-地址
                Route::get('/order/{id}',$controller . '@order')->name('agent.' . $config . '.order');// 博物馆-预订
                Route::get('/content/{id}', $controller . '@goodsContent')->name('agent.' . $config . '.content');// 博物馆-商品详情
            });
        }
    });
    //dark订单详情
    Route::get('/shop/dark/order_detail/{id}', 'DarkTemplateController@orderDetail')->name('agent.shop.dark.order_detail');
    ...
    foreach ($list as $v) {
        ...
        Route::group([//
            ...
            Route::get('/shop/buyTemplate','ShopController@buyTemplate')->name('shop.buy.template');// 店铺-选择模板-购买模板
            ...
            Route::any('/shop/save/template','ShopController@save')->name('shop.save.template');// 店铺-保存发布模板
