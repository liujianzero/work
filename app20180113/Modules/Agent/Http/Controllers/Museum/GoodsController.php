...
class GoodsController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($get = $request->get('screen')) {
            switch ($get) {
                case 'Y':
                    $list->where('goods_number', '>', 0)->where('is_on_sale', 'Y');
                    break;
                case '-1':
                    $list->where('goods_number', 0)->where('is_on_sale', 'Y');
                    break;
        ...
        $count = ModelsContentModel::from('models_content as mc')
            ->select([
                DB::raw("count(if(`{$mysql_prefix}mc`.`goods_number` > 0 and `{$mysql_prefix}mc`.`is_on_sale` = 'Y', true, null)) as on_sale"),
                DB::raw("count(if(`{$mysql_prefix}mc`.`goods_number` = 0 and `{$mysql_prefix}mc`.`is_on_sale` = 'Y', true, null)) as clean"),
