...
class FileClass {
	static function uploadModel($files, $path = 'default', $uid, $time) {
		$fileTypeArr = [ 
				'task',
				'sys',
				'user',
		...
		switch ($path) {
			...
			case 'models' :
				$disk = 'public';
				$filePath = $attachmentPath . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . $uid . DIRECTORY_SEPARATOR . $time . DIRECTORY_SEPARATOR;
				break;
			default :
				$disk = 'local';
				$filePath = $attachmentPath . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . date ( 'Y/m/d' ) . DIRECTORY_SEPARATOR;
		...
		foreach ( $files as $file ) {
			...
			if ($extension == "dom" || $extension == "jpg" || $extension == "png") {
				$tempPath = $filePath . "Image" . DIRECTORY_SEPARATOR;
			}else{
				$tempPath = $filePath;
			}
			...
			// if($file['ext'] =='jpg'|| $file['ext'] =='png' || $file['ext'] =='11dom'||$file['ext'] =='dom'){
			// $subpath = $this->getSubPath1($file['name']);
			// }else{
...
// 				}
				...
				$status = Storage::disk ( $disk )->put ( $tempPath . $clientName, file_get_contents ( $file->getRealPath () ) );
				$data = array ();
				if ($status) {
					
					$data ['url'] = $tempPath . $clientName;
	...
	static function uploadFile($file, $path = 'default', $allowExtension = null) {
		...
		switch ($path) {
			...
			case 'models' :
				$disk = 'public';
				$filePath = $attachmentPath . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . date ( 'Y/m/d' );
				break;
			default :
				$disk = 'local';
				$filePath = $attachmentPath . DIRECTORY_SEPARATOR . $path . DIRECTORY_SEPARATOR . date ( 'Y/m/d' ) . DIRECTORY_SEPARATOR;
